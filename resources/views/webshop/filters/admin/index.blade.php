            ...
            <div class="d-flex">
                <div class="mr-4 d-flex">
                    ...
                    <select name="" class="form-control form-control-sm with-selected">
                        ...
                        <option value="{{ route('admin.webshopFilter.delete_selected') }}">Verwijder geselecteerde</option>
                    </select>
                </div>
                <div>
                    <a href="{{ route('admin.webshopFilter.create') }}" class="btn btn-primary btn-sm">Filter toevoegen</a>
                </div>
            </div>
        </div>
    </div>
...
<div class="container">
    <table class="table" data-toggle="checkboxes" data-range="true">
        ...
        <tbody class="sortable" data-action="{{ route('admin.webshopFilter.sort') }}">
            @foreach ($objects as $obj)
                <tr id="{{ $obj->id }}">
                    <td><input type="checkbox" name="ids[]" class="check" value="{{ $obj->id }}"></td>
                    <td><a href="{{ route('admin.webshopFilter.edit', $obj) }}">{{ $obj->title }}</a></td>
